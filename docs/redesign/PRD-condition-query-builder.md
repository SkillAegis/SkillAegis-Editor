...
### 6.1 `extract_type` coupling — ✅ IMPLEMENTED
...
**As shipped** (`ConditionRow.vue` + `ComparisonOperatorsHelp.vue`): picking `count` auto-sets `extract_type = 'all'` (a watcher on the comparison — fires on the author's choice, never on load, so untouched scenarios don't churn). In builder mode a `count` condition shows a read-only **"counts every match"** note instead of the checkbox; a legacy `count` still stored with `first` is surfaced as a one-click **"only counting the first match — fix"** warning (this counts one item's *fields*, not the matches — the exact latent bug in 3 shipped conditions). Non-`count` comparisons keep an explicit, renamed opt-in — **"check every match, not just the first"** with a help tooltip — rather than removing control entirely. Raw mode keeps the original checkbox (relabelled with the `extract_type` hint). The **operators help modal** gained a *"Matching scope — first vs. all"* section documenting what `extract_type` does, since it decides which per-type table (string / list / object) applies.
...
3. **P3 — Response & polish. ✅ COMPLETE.** `resp-*` presets for `query_search`/`misp_query_search` (Q3), `is one of` / `matches`, "match all like this" ↔ `extract_type`, placeholder pass-through — all shipped. The `extract_type` coupling (§6.1) landed last: `count` auto-uses `all` (with the manual checkbox hidden in builder mode and a fixable warning for legacy `count`-with-`first` conditions), non-`count` comparisons keep a renamed **"check every match, not just the first"** opt-in, and the operators help modal documents `first` vs `all`. Verified: eslint clean, `vite build` green, property test PASS (no core change), **Playwright 9/9** on the read-only sandbox (`:4055` → sandbox scenario copy; count auto-note + renamed toggle render, flipping a comparison to `count` auto-adds the note, the legacy `.Event.Object[]|select(.name=="domain-ip")` count shows and one-click-fixes the warning, the modal documents "Matching scope"; 0 console errors, sandbox byte-identical after).
