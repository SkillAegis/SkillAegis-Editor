...
- **M1 (coverage)** ≥ 70% of library condition paths *by occurrence* open in builder mode. Measured progression, each slice keeping the §12 invariant (every path parses to a semantically-equal query **or** round-trips to raw **verbatim** — 0 rewrite errors): P1 single-level grammar **72%** occ / 56% distinct → **+ two-level object→attribute** **78%** occ / 68% distinct → **+ Tag/Note slice** (tag-name predicate, null-safe "has any tag", Note collection) **82.8%** occ (144/174) / **75.9%** distinct (60/79). Remaining fallbacks are map/group_by pipelines, sub-array projections (`… | .Tag[].name`), unmodelled root arrays (webhook/suricata `.[]…`, `._secret`) and the two irreducible monsters.
