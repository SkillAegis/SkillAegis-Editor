...
|---|----------|------------------|
| OQ1 | Should the builder also cover `simulate_ips` (`.[].alert.signature`, `.[].verdict.action`) sources? | Yes, add an `alerts` preset kind in a later slice; low volume, simple shape. |
...
| OQ4 | Autocomplete filter/projection field names from a live sample event's keys? | Nice-to-have; fold into concept ② (point-at-data) rather than here. |

---

## 15. Success metrics

- **M1 (coverage)** ≥ 70% of library condition paths *by occurrence* open in builder mode (**measured: 72%** with the P1 single-level grammar; 56% of distinct paths); the rest round-trip to raw **verbatim** (0 rewrite errors — the §12 property test passes on the whole library). A later slice that adds two-level selects + multi-level projections would lift this toward ~90%.
...
1. **P1 — Core + fallback. ✅ COMPLETE.**
   ...
   - ⏭ **Fast-follow (was in P1's spirit, deferred by scope call):** two-level object→attribute selects (the top fallback cluster) — see §14 OQ / §2 table.
2. **P2 — Live preview + validation.** Inline `/injects/jq-path-test` per condition; validity indicator + extracted-value preview + plain-language sentence.
...
4. **P4 (optional) — `simulate_ips` sources** (OQ1) and the hooks for concept ② to pre-fill the slots.
